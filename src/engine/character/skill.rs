    ...
    pub fn toggle (&mut self) -> bool {
        match self.cooldown {
            Capacity::Constant (TOGGLE_OFF, t1, t2) => {
                ...
                self.status_ids[STATUS_ACTIVE] = if self.status_ids[STATUS_ACTIVE] == self.status_ids[STATUS_0] {
                    assert_eq! (t1, TOGGLE_ON);
                    assert_eq! (t2, TOGGLE_OFF);

                    toggle_second = TOGGLE_ON;

                    self.status_ids[STATUS_1]
                } else {
...
impl Applier for Skill {
    fn try_yield_appliable (&self, lists: Rc<Lists>) -> Option<Box<dyn Appliable>> {
        let status: Status = lists.get_status (&self.status_ids[STATUS_ACTIVE]).clone ();
